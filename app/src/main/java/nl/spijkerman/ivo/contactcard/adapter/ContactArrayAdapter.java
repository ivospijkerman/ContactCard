...
//        Contact contact = ContactController.INSTANCE.getById(position);
        ...
        boolean useRepo = ctx.getSharedPreferences(ctx.getString(R.string.SOURCE_PREF), 0)
                .getBoolean(ctx.getString(R.string.USE_REPO), true);
        ContactSource contactSource = useRepo
                ? new ContactRepository(this.getContext())
